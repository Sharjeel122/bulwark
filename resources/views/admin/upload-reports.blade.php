...
{{--                        <td><a href="{{ asset('assets/reports/'.$report->report) }}" DOWNLOAD>{{$report->report}}</a></td>--}}
                        <td>
                            ...
                            <a href="{{ route('admin.edit_report', $report->id) }}" class="btn btn-sm btn-outline-primary" title="Edit Report"><i class="fa fa-edit"></i></a>
                        </td>
                    </tr>
                    @endforeach
    ...
    <div class="modal fade" id="uploadreport" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <form action="{{route('admin.upload_report')}}" method="post" enctype='multipart/form-data'>
                    @csrf
                    <div class="modal-body">
                        <input type="text" name="report_title" value="" placeholder="Report Title" class="form-control" required>
                        <input type="hidden" name="staff_id" value="{{Auth::user()->id}}">
                        <select name="user_id" id="user" class="form-control mt-4" required>
